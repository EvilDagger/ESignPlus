...
4. This isn't a bug but it's a small nuisance. The bundle identifier is still xyz.yyyue.esign and a few other details (not the name) are eSign. This is especially true when installing the IPA but it also happens with the traditional method. A fix must be rolled down in the beta phase!
...
10. If you are at located at the **Apps manager** in Filza and haven't followed steps 8.1-8.4, find the system app that you have chosen to replace with the helper app. If you cannot find it, keep scrolling in the app list. After finding, click the **ⓘ** button and a menu of the app should pop up. When the menu of items pops up, click the Bundle button which should have a value as it's path. When clicked, you will go to the application's directory.
11. When the system app is opened, you should see the **Books.app** folder (or **Calculator.app**, in other words the name of the system app that was chosen). You may see a couple of .plist files, ignore them. Now, open the folder by clicking it.
12. When you're at the folder, click the **Pasteboard** button (file with three dots) and if you have done the fifth step right, an dialog shall appear saying: **An item named "Books" already exists in this location. Do you want to replace it with the one you're moving?** (the **Books** item may be different depending on the system app chosen). When that happens, click the **Replace** button! If that dialog hasn't appeared, it signifies that the process was done unsuccessfully meaning you have to delete everything related to eSign+ and restart the whole process. The system app will get replaced with the helper app's UI.
**The steps that are about to follow are very crucial!**
13. Once the modified file replaces the system file, click the **ⓘ** button and a menu should pop up. Firstly, you need to change it's ownership permissions. Go to the **Ownership** section and you should see two menu options: **Owner** and **Group**. Click the **Owner** (or **Group**) menu option and then have the owner become **33. _installd**! Scroll down to the **Group** section and change their permission level to **33. _installd**. Click the **Save** button and then return back to the menu. **Keep in mind that you cannot modify the Owner and Group permissions in different menus, you need to do this on the same menu!**. You should choose **_installd** specifically and not experiment with other **daemons**, **users** or **groups** due to security concerns and other nitpicks! This is meant to actually give installation privileges.
14. Once that is done, go to the **Access Permissions**, click the **Sticky** menu option and click the **Mask** numeric field. This can be easily used to set up and modify permissions for the owner, group and everyone else. Click on the number and change it entirely (if this is difficult, click the number two times/select it and then overwrite it) to: **0755**. This makes the installation process possible.
15. We haven't yet imported the **esignhelper** file which provides the code and functionality of the helper app. To do that, switch to the other Filza window by clicking the same button you have clicked on step 7 of the tutorial (box in front of the other box). After doing that, switch to the other window with the **esignhelper** file and the system app which we are done with.
16. Copy **esignhelper** in the same way as step 6 (**hold** or **long press** the file and press the **Copy** button in the menu).
17. Go back to the other tab and paste the file. This file is also required and serves as a library to the app you have just been done modifying (the helper app). No dialog should appear this time!
18. Do steps 13-14 on the file you have copied. In other words, change it's owner and group to **33. _installd** and change it's sticky note to **0755** (rwx for owner, r-x for group, r-x for others)
